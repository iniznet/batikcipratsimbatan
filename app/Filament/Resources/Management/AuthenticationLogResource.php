...
use App\Filament\Resources\Management\AuthenticationLogResource\Pages;
use App\Filament\Resources\Management\AuthenticationLogResource\RelationManagers;
use Filament\Forms;
...
class AuthenticationLogResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('authenticatable')
                    ...
                    ->formatStateUsing(function (?string $state, Model $record) {
                        ...
                        $username = $record->authenticatable->username ?? $record->authenticatable->email;

                        return new HtmlString('<a href="'.route('filament.admin.resources.'.Str::plural((Str::lower(class_basename($record->authenticatable::class)))).'.edit', ['record' => $record->authenticatable_id]).'" class="inline-flex items-center justify-center text-sm font-medium hover:underline focus:outline-none focus:underline filament-tables-link text-primary-600 hover:text-primary-500 filament-tables-link-action">'.$username.'</a>');
                    })
                    ->sortable(),
            ...
            ->filters([
                Tables\Filters\Filter::make('login_successful')
                    ->toggle()
                ...
                Tables\Filters\Filter::make('cleared_by_user')
                    ->toggle()
    ...
    {
        return [\App\Models\Management\User::class];
    }
}
